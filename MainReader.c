 ...
 * (Gcc, VSCode, Codeblocks, etc.). bErrorPrint
 *.............................................................................
 */
 ...
 * -------------------------------------------------------------
 */
PowerR_void bErrorPrint(PowerR_string fmt, ...);
PowerR_void displayBuffer(Buffer* ptr_Buffer);
PowerR_long getFileSize(PowerR_string fname);
PowerR_int isNumber(const PowerR_string ns);
PowerR_void startReader(PowerR_string, PowerR_string, PowerR_char, PowerR_int, PowerR_int);
...
PowerR_int mainReader(PowerR_int argc, PowerR_string* argv) {
	...
	PowerR_string program = argv[0];
	PowerR_string input = argv[2];
	PowerR_char mode = MODE_FIXED;//CHANGE MODE HERE
	PowerR_int size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
PowerR_void startReader(PowerR_string program, PowerR_string input, PowerR_char mode, PowerR_int size, PowerR_int increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	PowerR_int loadSize = 0;		/* the size of the file loaded in the buffer */
	PowerR_char symbol;			/* symbol read from input file */

	/* Create buffer */
	bufferp = readerCreate(size, (char)increment, mode);/*Data Type of Parameter increament and mode are wrong??????? */
...
PowerR_void bErrorPrint(PowerR_string fmt, ...) {
	...
	(PowerR_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
